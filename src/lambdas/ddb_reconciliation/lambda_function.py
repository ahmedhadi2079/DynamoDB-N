...
def read_athena(sql_path: str, input_database: str) -> pd.DataFrame:
    ...
    with open(sql_path, "r") as total_onboarding_status_timeline_sql_file:
        total_onboarding_status_timeline_sql = total_onboarding_status_timeline_sql_file.read()
...
def generic_construct_count_dataframe(athena_df: pd.DataFrame, dynamo_count: int) -> pd.DataFrame:
    """
    ...
    return pd.DataFrame(data, columns=["count_dynamo", "count_athena", "date", "timestamp_extract"])
...
def recon_check_counts(dynamo_count: int, athena_count: int, threshold: float) -> bool:
    ...
    if os.environ.get("IS_PRODUCTION"):
        if perc_dynamo_count > athena_count:
            logger.error(f"Row Counts Do Not Match: Data Lake Total Count = {athena_count}; Dynamo Total Count = {dynamo_count}")
            return False
        else:
            logger.info(f"Row Counts Match or within defined threshold: Data Lake Total Count = {athena_count}; Dynamo Total Count = {dynamo_count}")
            return True
    else:
        if perc_dynamo_count > athena_count:
            logger.warn(f"Row Counts Do Not Match: Data Lake Total Count = {athena_count}; Dynamo Total Count = {dynamo_count}")
            return False
        else:
            logger.info(f"Row Counts Match: Data Lake Total Count = {athena_count}; Dynamo Total Count = {dynamo_count}")
            return True


def construct_customers_count_dataframe(athena_df: pd.DataFrame, dynamo_count: int) -> pd.DataFrame:
    """
    ...
    agg_athena = athena_df.groupby(by=["status_", "onboarding_status_date_"]).count().reset_index()
...
def write_reconcilication_to_s3(
    ...
    dynamo_athena_merged_df = construct_count_dataframe(athena_df, latest_scanned_count_dynamo_df)
...
def process_customers_reconciliation():
    sql_filepath = "5710_total_onboarding_timeline.sql"
    input_db = "datalake_raw"
    logger.info("Starting customers reconciliation. ")
    athena_df = read_athena(sql_filepath, input_db)
    latest_scanned_count_dynamo_df = read_dynamo_aws_cli(os.environ["DYNAMODB_CUSTOMERS"])
    write_reconcilication_to_s3(
        construct_customers_count_dataframe,
        athena_df,
        latest_scanned_count_dynamo_df,
    ...
    logger.info("Finished customers reconciliation. ")


def generic_process_reconciliation(sql_filepath: str, athena_table_name: str, dynamo_table_name: str, threshold: float):
...
def lambda_handler(event, context):
    # customers recon: This is slightly different curerrently than all others
    process_customers_reconciliation()
    # generic recon functions
    for dynamo_recon_table in config.dynamo_tables.keys():
        generic_process_reconciliation(
            config.dynamo_tables[dynamo_recon_table]["reconcile_sql_path"],
            dynamo_recon_table,
            os.environ.get(config.dynamo_tables[dynamo_recon_table]["dynamo_table_env_var_name"]),
